        ...
        <article class="lg:col-span-8">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                @if($job->converted_images)
                    <div class="p-10 space-y-6 bg-slate-50">
                        @foreach($job->converted_images as $img)
                            <img src="{{ url('career/' . basename($img)) }}" class="w-full rounded-xl">
                        @endforeach
                    </div>
                @elseif($job->file_path && !str_ends_with($job->file_path, '.pdf'))
                    <div class="p-8">
                        <img src="{{ url('career/' . basename($job->file_path)) }}" class="w-full rounded-xl">
                    </div>
                @endif
